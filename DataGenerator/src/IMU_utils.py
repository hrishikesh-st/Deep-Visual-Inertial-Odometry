...
def run_acc_demo(ref_a):
    fs = 100  # Hz
    num_samples = 1000
    # ref_a = np.zeros((num_samples, 3))

    # clover_trajectory(np.linspace(0, 10, 1000))
    # for i in range(1000):
    #     x = i +1
    #     y = i +2
    #     z = i + 3
...
def run_gyro_demo(ref_g):
    fs = 100  # Hz
    # num_samples = 10
    # ref_w = np.zeros((num_samples, 3))
    # print(ref_w)
    gyro_err = gyro_low_accuracy
    ...
    real_gyro = gyro_gen(fs, ref_g, gyro_err, vib_def)
